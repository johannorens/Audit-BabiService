...
function PrestatairesAdmin() {
  ...
  useEffect(() => {
    Promise.all([apiGetPrestataires(), apiGetCategories()]).then(
      ([prestatairesRes, categoriesRes]) => {
  ...
  async function handleSubmit(e) {
    ...
    const prestataire = data.data ?? data;

    if (editingId) {
      setPrestataires((prev) =>
        prev.map((p) => (p.id_prestataire === editingId ? prestataire : p)),
      );
    } else {
      setPrestataires((prev) => [prestataire, ...prev]);
    }
    setModalOpen(false);
  }
            ...
            <tbody>
              {prestataires.map((prestataire) => (
                ...
                >
                  ...
                  <td className="py-3 pr-4">
                    <span
                      className={`text-xs font-semibold px-3 py-1 rounded-full ${statutStyles[prestataire.statut]}`}
                    >
