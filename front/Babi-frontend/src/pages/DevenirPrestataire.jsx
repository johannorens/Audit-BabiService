...
function DevenirPrestataire() {
  ...
  async function handleSubmit(e) {
    ...
    if (!ok) {
      setError(
        data.message ||
          "Impossible d'envoyer votre candidature, vérifiez vos informations.",
      );
      return;
    }
    setSubmitted(true);
    ...
    <div className="min-h-screen flex flex-col bg-white">
      ...
      <main className="flex-1 bg-babi-cream">
        <div className="max-w-3xl mx-auto px-6 py-12">
          <Link
            to="/"
            className="inline-flex items-center gap-2 text-gray-500 hover:text-babi-green transition-colors mb-6"
          >
            ← Accueil
          </Link>
          ...
          <p className="text-gray-600 mb-8 max-w-xl">
            Remplissez ce formulaire, notre équipe valide votre profil sous 24h
            et vous contacte pour la suite.
          </p>
          ...
          <div className="flex flex-wrap gap-4 mb-10">
            {checklist.map((item, index) => (
              ...
              >
                ...
                <span className="text-sm text-babi-dark font-medium">
                  {item}
                </span>
              </div>
            ))}
          ...
          {submitted ? (
            <div className="bg-white rounded-2xl p-8 text-center">
              ...
              <h2 className="text-xl font-bold text-babi-dark font-bricolage mb-2">
                Candidature envoyée !
              </h2>
              <p className="text-gray-600 mb-6">
                On revient vers vous par email ou téléphone sous 24h une fois
                votre profil validé.
              </p>
              <Link
                to="/"
                className="inline-block bg-babi-green text-white px-6 py-3 rounded-full font-semibold hover:-translate-y-0.5 hover:shadow-lg transition-all"
              >
                Retour à l'accueil
              </Link>
            </div>
