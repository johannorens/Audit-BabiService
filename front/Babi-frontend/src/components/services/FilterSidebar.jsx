...
export default function FilterSidebar({ filters, onChange, onReset }) {
  ...
  return (
    <aside className="w-full md:w-[260px] flex-shrink-0 space-y-8">
              ...
              <li key={id_categorie}>
                <button
                  onClick={() =>
                    onChange({
                      ...filters,
                      categorie: active ? null : nom_categorie,
                    })
                  }
      ...
      {/* Note minimale */}
      <div>
        <h3 className="text-sm font-semibold text-[#0B2B26] mb-3">
          Note minimale
        </h3>
      ...
      <div className="bg-[#F0F7F4] rounded-xl p-4 flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-[#7A9C90] mt-0.5">
            Masquer les pros occupés
          </p>
        </div>
        <button
          type="button"
          ...
          onClick={() =>
            onChange({
              ...filters,
              disponiblesSeulement: !disponiblesSeulement,
            })
          }
        ...
        >
          <span
            className={`absolute top-0.5 left-0.5 w-5 h-5 rounded-full bg-white shadow transition-transform ${disponiblesSeulement ? "translate-x-5" : ""}`}
          />
        </button>
      </div>
